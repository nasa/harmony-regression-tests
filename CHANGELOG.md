...
- Fixed l2ss-py regression reference files for new l2ss-py on how fill values are handled
...
- Add auto-tagging of test image version when test runs successfully.


## 2026-09-16 ([#319](https://github.com/nasa/harmony-regression-tests/pull/319))

### Changes

- Updates HOSS regression test library versions.
- Updates HOSS to ruff 0.16.x ruleset.

## 2026-09-08 ([#321](https://github.com/nasa/harmony-regression-tests/pull/321))

### Changed

- Updated the sambah reference hashes for changes introduced by updates in l2ss-py 3.3.3.
...
- Update `nsidc-icesat2` tests to use granule_name instead of granule_id.
- Update from ATL24 v1 to v2 in `nsidc-icesat2`.
- Remove ATL12 prod test in `nsidc-icesat2` due to revised granule ingest and removal of existing granule.
- Update `nsidc-smap` tests to use granule_name instead of granule_id.
...
- Updated expected outputs of nisar-py test to reflect changes in nisar-py v0.4.0.

## 2026-05-19 ([#289](https://github.com/nasa/harmony-regression-tests/pull/289))

### Changed

- Updated ATL10 reference granule for `nsidc-icesat2` tests.
...
- Updates the underlying docker image that the tests run on and increases the patch version on all tests.
...
- Updated the CASPER reference files to reflect the recent CASPER changes to the output
  filenames.

## 2026-03-02 ([#255](https://github.com/nasa/harmony-regression-tests/pull/255))
...
- Created a regression test suite for the giovanni-averaging-service
...
## 2025-08-14 ([#202](https://github.com/nasa/harmony-regression-tests/pull/202))
